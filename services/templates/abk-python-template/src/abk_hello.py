...
def get_error_response_body(event_body:dict) -> AhLambdaResponseBody:
    ...
    """
    abk_logger.info(f"-> get_error_response_body()")
    resp_body = AhLambdaResponseBody(
        msg="error",
        txId=event_body.get('txId', '')
    )
...
def handler(event, context):
    ...
    abk_logger.debug(f"context = {json.dumps(context, default=lambda o: getattr(o, '__dict__', str(o)))}")
    resp_body: AhLambdaResponseBody
