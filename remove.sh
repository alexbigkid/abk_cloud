...
{
    echo
    echo "$0 run all scripts to remove Cloud Infrastructure"
...
#execute sub scripts
SCRIPT_FILES=$(find . -maxdepth 1 -name 'remove-[0-9][0-9][0-9]*.sh' -type f | sort -r)
...
for SCRIPT in ${SCRIPT_FILES}; do
    ...
    "$SCRIPT" "$ABK_DEPLOYMENT_ENV" "$ABK_DEPLOYMENT_REGION" || exit $?
done
