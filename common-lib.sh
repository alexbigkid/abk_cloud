...
ABK_USERS_POOL_NAME="AbkUserPool"
ABK_USERS_POOL_CLIENT_NAME="AbkAppClient"
...
# color definitions
#------------------------------------------------------------------------------
if [ -z "${ABK_DISABLE_BASH_COLORS+x}" ] || [ -z "$ABK_DISABLE_BASH_COLORS" ]; then
    BLK='\033[0;30m'
    RED='\033[0;31m'
    GRN='\033[0;32m'
    ORG='\033[0;33m'
    ...
    echo "not using colors: ABK_DISABLE_BASH_COLORS is defined: $ABK_DISABLE_BASH_COLORS"
    BLK=
    RED=
    GRN=
    ORG=
...
PrintTrace() {
    ...
    if [ $TRACE_LEVEL -ge $LCL_TRACE_LEVEL ]; then
        case $LCL_TRACE_LEVEL in
            ...
            "$TRACE_WARNING")
                echo -e "${YLW}[WARNING] ${LCL_PRINT_STRING[*]}${NC}" ;;
            "$TRACE_FUNCTION")
...
use_defined_variables() {
    ...
    PrintTrace $TRACE_NONE "$ENV_ARRAY $REGION_ARRAY $ABK_USERS_POOL_NAME $ABK_USERS_POOL_CLIENT_NAME"
}
...
WriteValueToConfigFile() {
    ...
    if [ $# -eq 3 ] && [ -f "$LCL_ENV_CFG_FILE" ]; then
        ...
        if [ "$ABK_UNIX_TYPE" = "mac" ]; then
            ...
            PrintTrace $TRACE_ERROR "$ABK_UNIX_TYPE is not supported"
        fi
    fi
...
GetCognitoIdToken() {
    ...
    LCL_AUTH_COGNITO_ID_TOKEN=$(aws cognito-idp initiate-auth --auth-flow USER_PASSWORD_AUTH --auth-parameters USERNAME="$ABK_COGNITO_USR",PASSWORD="$ABK_COGNITO_PSW" --client-id "$LCL_USER_POOL_CLIENT_ID" --output text --query 'AuthenticationResult.IdToken')
...
InstallPythonTestDependenciesIfRequired() {
    ...
    # Python dependencies needs to be installed everytime on the pipeline, since they will be missing.
